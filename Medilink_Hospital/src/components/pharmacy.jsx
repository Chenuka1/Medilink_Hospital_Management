...
export default function Pharmacy() {
    ...
    const fetchPharmacyPatients = async () => {
      try {
        const response = await axios.get(
          `https://localhost:7132/api/Treatment/preparationcomplete`
        );
  ...
  const fetchTreatmentDetails = async (patientId, serialNo) => {
    try {
      const response = await axios.get(
        `https://localhost:7132/api/Treatment/patient/record/${patientId}/${serialNo}`
      );
  ...
  const handleSaveAndProceed = async () => {
    try {
      await axios.patch(
        `https://localhost:7132/api/Treatment/update/status/${selectedPatient.MPD_PATIENT_CODE}/${selectedPatient.MTD_SERIAL_NO}`
      );
          ...
          <tbody>
            {filteredPatients.map((patient, index) => (
              <tr key={index}>
                ...
                <td>{patient.MTD_TREATMENT_STATUS === "P"
                  ? "Preparation Complete"
                  : patient.MTD_TREATMENT_STATUS}</td>
                <td>
                  <button
        ...
        <div className="popup-overlay">
          <div className="popup-content">
            ...
            <div className="popup-buttons">
              ...
              <button onClick={handleSaveAndProceed} className="save-btn">
                Save & Proceed to Invoice
              </button>
            </div>
          </div>
        </div>
